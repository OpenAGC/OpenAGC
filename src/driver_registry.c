    ...
    AgcProsperoDirectProfile *profile_out)
{
     ...
     * submission, not Sony's incompatible multi-argument export ABI. FW 5.50
     * retains its hardware-qualified extension. The exact Sony stream adapter
     * still stalls FW 11.60 even after its process property is installed, so
     * that firmware remains fail-closed. EOP flip and suspend query retain
