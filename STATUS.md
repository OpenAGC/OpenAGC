...
1. **Milestone 5 is complete.** Runtime API v23 has the optional
   ...
   matching host decoder. The host invalid-program matrix now closes creation,
...
The current Vulkan migration is host-complete at the native ownership boundary.
Its mechanically checked low-level inventory is zero: descriptor encoding,
image layout, tessellation resources, command storage, finite fence waits,
submission, and presentation use public native APIs, and the legacy encoder
and fallback are deleted. Normal and sanitizer host qualification pass 46/46,
...
`agc_runtime_graphics.elf` probe. Its two color exports compile with matching
...
and no `eboot.bin` process remained after the stopped attempt. FW 11.60 passed
a one-launch canary and 14/14 stress launches with a GPU fence, driver shutdown,
and all four cleanup results zero on every iteration; websrv and ps5debug-NG
remained reachable. FW 5.50 then passed the corrected canary and full 14/14
threshold sequence with the same exact cleanup invariants. See
...
requires `49152` and reproduced the exact count, D32 classes, bounded
completion, clean shutdown, and no residual process. The complete D32 HTILE
tier is hardware-qualified on both endpoints. Combined D32+S8 ordinary HTILE
is now the active compressed-depth gate.
...
After D32 endpoint qualification, all eight combined artifacts were relinked
...
residual process. Its recipe now freezes `49152`; the identical replay
reproduced every invariant. The pinned FW 5.50 depth-only mirror now requires
the same count and reproduced every aspect-RMW, metadata, allocation-aware
D32, S8, fence, shutdown, and cleanup invariant. Depth-only combined expclear
is hardware-qualified on both endpoints. Stencil-only FW 11.60 pass 1 is now
active and reproduced aspect `0x2`, expected metadata `0xfffff0ff`, zero RMW
...
FW 5.50 partial-rate pass 2 reproduced every exact value and clean lifecycle.
...
FW 11.60 partial-rate pass 1 exactly matched FW 5.50's zero sample counters,
...
FW 5.50 array-layer-1 is now active. Its first guarded run passed, and an
...
Pinned RG32_SINT pass 1 succeeds with two 255,744-sample full-range lanes,
...
BC7 SRGB also passed twice with identical coverage/mode counts, alpha range
`0..255`, zero exact mismatches, channel independence, and FNV64
`0x74a3526f9a3eef65`. Both BC7 variants are hardware-qualified on exact
FW 11.60.
BC5 SNORM also passed twice with equal coverage/region counts, full remapped
R/G ranges, zero decode mismatches/error, channel independence, and FNV64
`0xf1464077ada8ce8d`. Both BC5 variants are hardware-qualified on exact
FW 11.60.
...
remains pending a clean FW 11.60 boot. BC2 UNORM then passed twice with
identical `224640` changed pixels, equal region counts, alpha range `0..255`,
zero exact mismatches, and FNV64 `0xf3b07b5935bb483d`. BC2 UNORM is
hardware-qualified on exact FW 11.60.
...
BC3 SRGB also passed twice with equal coverage/region counts, alpha range
`0..255`, zero exact mismatches, and FNV64 `0x4cef62aedf2c8da1`. Both BC3
variants are hardware-qualified on exact FW 11.60.
...
BC6 UFLOAT on FW 5.50 used the exact FW 11.60-qualified bytes twice and
...
BC7 UNORM on FW 5.50 used the exact FW 11.60-qualified bytes twice and
...
BC3 SRGB on FW 5.50 twice reproduced the exact FW 11.60-qualified coverage,
equal regions, alpha range `0..255`, zero exact mismatches, and FNV64
`0x4cef62aedf2c8da1`. All lifecycle gates passed. Both BC3 variants are
hardware-qualified on exact FW 5.50 and FW 11.60.
...
gates passed. BC2 UNORM is hardware-qualified on both endpoints; SRGB is next.

BC2 SRGB on FW 5.50 twice reproduced the exact FW 11.60-qualified coverage,
equal regions, alpha range `0..255`, zero exact mismatches, and FNV64
`0x0a8a977e6f2c8da1`. All lifecycle gates passed. Both BC2 variants are
hardware-qualified on exact FW 5.50 and FW 11.60.
