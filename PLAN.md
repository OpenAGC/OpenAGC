...
| 5. Publish a homebrew-facing example | **In progress** | `samples/triangle` now demonstrates atomic composition of the public frame, Wave32 baseline draw, and render-to-present APIs. It still needs integration into a standalone Prospero application with allocation, shader upload, bounded completion, and VideoOut presentation before this gate is complete. Retail import audits remain bounded ABI evidence only. |
