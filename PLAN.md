...
| 5. Publish a homebrew-facing example | **In progress** | `samples/triangle` now demonstrates atomic composition of the public frame, Wave32 baseline draw, and render-to-present APIs and cross-builds with the Prospero toolchain. The focused `agc_graphics.elf` qualification payload is rebuilt and ready for curl/websrv. Standalone application allocation, shader upload, bounded completion, and VideoOut presentation remain before this gate is complete. Retail import audits remain bounded ABI evidence only. |
