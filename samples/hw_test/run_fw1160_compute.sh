...
COMPUTE_ARTIFACT=${COMPUTE_ARTIFACT:?set COMPUTE_ARTIFACT}
PROCESS_CLEANUP_ELF=${PROCESS_CLEANUP_ELF:?set PROCESS_CLEANUP_ELF}
REMOTE_BASE=/data/homebrew/openagc_fw1160_compute

if [ ! -s "$COMPUTE_ARTIFACT" ] || [ ! -s "$PROCESS_CLEANUP_ELF" ]; then
    echo "missing compute-test or process-cleanup ELF" >&2
    exit 2
fi
...
curl -sS --fail --ftp-create-dirs -T "$COMPUTE_ARTIFACT" \
...
grep -q "Total: 2073600/2073600 pixels match" "$output_file" || exit 1
grep -q "Driver shutdown: PASS" "$output_file" || exit 1
grep -q "Compute result: PASS" "$output_file" || exit 1
if grep -Eq "FAIL|FATAL|MISMATCH" "$output_file"; then
    exit 1
fi
if [ "$transport_status" -ne 0 ]; then
    echo "compute passed; websrv transport ended with curl=$transport_status" >&2
fi
