...
case "$EXPECTED_TARGET" in
    ...
    RG32_UINT)
        grep -Eq '^\[UINT32 Lane 0\].*: PASS$' "$output_file" || exit 1
        grep -Eq '^\[UINT32 Lane 1\].*: PASS$' "$output_file" || exit 1
        grep -Eq '^\[UINT32\] changed=[1-9][0-9]*/[1-9][0-9]* .* independence=PASS: PASS$' \
            "$output_file" || exit 1
        ;;
    RGBA32_UINT)
        grep -Eq '^\[UINT32 Lane 0\].*: PASS$' "$output_file" || exit 1
        grep -Eq '^\[UINT32 Lane 1\].*: PASS$' "$output_file" || exit 1
        grep -Eq '^\[UINT32 Lane 2\].*: PASS$' "$output_file" || exit 1
        grep -Eq '^\[UINT32 Lane 3\].*: PASS$' "$output_file" || exit 1
        grep -Eq '^\[UINT32\] changed=[1-9][0-9]*/[1-9][0-9]* .* independence=PASS: PASS$' \
            "$output_file" || exit 1
        ;;
    R32_SINT)
    ...
    RGBA32_SINT)
        grep -Eq '^\[SINT32 Lane 0\].*: PASS$' "$output_file" || exit 1
        grep -Eq '^\[SINT32 Lane 1\].*: PASS$' "$output_file" || exit 1
        grep -Eq '^\[SINT32 Lane 2\].*: PASS$' "$output_file" || exit 1
        grep -Eq '^\[SINT32 Lane 3\].*: PASS$' "$output_file" || exit 1
        grep -Eq '^\[SINT32\] changed=[1-9][0-9]*/[1-9][0-9]* .* independence=PASS: PASS$' \
            "$output_file" || exit 1
        ;;
    BC1_UNORM)
    ...
    BC1_SRGB)
        ...
        ;;
    BC4_UNORM)
        grep -Eq '^\[BC4 UNORM\] changed=[1-9][0-9]* regions=\{[1-9][0-9]*,[1-9][0-9]*,[1-9][0-9]*\} decode-mismatches=0 max-error=0 packed-fnv64=0x[0-9a-f]{16}: PASS$' \
            "$output_file" || exit 1
        grep -q '^\[BC4\] mip0/mip1/layer1 signed-range selection: PASS$' \
            "$output_file" || exit 1
        ;;
    BC4_SNORM)
        grep -Eq '^\[BC4 SNORM\] changed=[1-9][0-9]* regions=\{[1-9][0-9]*,[1-9][0-9]*,[1-9][0-9]*\} decode-mismatches=0 max-error=[01] packed-fnv64=0x[0-9a-f]{16}: PASS$' \
            "$output_file" || exit 1
        grep -q '^\[BC4\] mip0/mip1/layer1 signed-range selection: PASS$' \
            "$output_file" || exit 1
        ;;
    BC2_UNORM)
        grep -Eq '^\[BC2 UNORM\] changed=[1-9][0-9]* regions=\{[1-9][0-9]*,[1-9][0-9]*,[1-9][0-9]*\} alpha-range=0\.\.255 exact-mismatches=0 packed-fnv64=0x[0-9a-f]{16}: PASS$' \
            "$output_file" || exit 1
        grep -q '^\[BC2\] mip0/mip1/layer1 alpha selection: PASS$' \
            "$output_file" || exit 1
        ;;
    BC2_SRGB)
        grep -Eq '^\[BC2 SRGB\] changed=[1-9][0-9]* regions=\{[1-9][0-9]*,[1-9][0-9]*,[1-9][0-9]*\} alpha-range=0\.\.255 exact-mismatches=0 packed-fnv64=0x[0-9a-f]{16}: PASS$' \
            "$output_file" || exit 1
        grep -q '^\[BC2\] mip0/mip1/layer1 alpha selection: PASS$' \
            "$output_file" || exit 1
        ;;
    ...
    BC3_SRGB)
        ...
        ;;
    BC5_UNORM)
        grep -Eq '^\[BC5 UNORM\] changed=[1-9][0-9]* regions=\{[1-9][0-9]*,[1-9][0-9]*,[1-9][0-9]*\} R=0\.\.255 G=0\.\.255 decode-mismatches=0 max-error=0 independence=PASS packed-fnv64=0x[0-9a-f]{16}: PASS$' \
            "$output_file" || exit 1
        grep -q '^\[BC5\] mip0/mip1/layer1 dual-channel selection: PASS$' \
            "$output_file" || exit 1
        ;;
    BC5_SNORM)
        grep -Eq '^\[BC5 SNORM\] changed=[1-9][0-9]* regions=\{[1-9][0-9]*,[1-9][0-9]*,[1-9][0-9]*\} R=0\.\.255 G=0\.\.255 decode-mismatches=0 max-error=[01] independence=PASS packed-fnv64=0x[0-9a-f]{16}: PASS$' \
            "$output_file" || exit 1
        grep -q '^\[BC5\] mip0/mip1/layer1 dual-channel selection: PASS$' \
            "$output_file" || exit 1
        ;;
    ...
    BC7_SRGB)
        ...
        ;;
    BC6_UFLOAT)
        grep -Eq '^\[BC6 UFLOAT\] changed=[1-9][0-9]* fixtures=\{[1-9][0-9]*,[1-9][0-9]*,[1-9][0-9]*,[1-9][0-9]*\} range=[0-9]+\.\.[0-9]+ decode-mismatches=0 max-error=[0-2] independence=PASS packed-fnv64=0x[0-9a-f]{16}: PASS$' \
            "$output_file" || exit 1
        grep -q '^\[BC6\] mip0/mip1/layer1 signed-mode selection: PASS$' \
            "$output_file" || exit 1
        ;;
    BC6_SFLOAT)
        grep -Eq '^\[BC6 SFLOAT\] changed=[1-9][0-9]* fixtures=\{[1-9][0-9]*,[1-9][0-9]*,[1-9][0-9]*,[1-9][0-9]*\} range=[0-9]+\.\.[0-9]+ decode-mismatches=0 max-error=[0-2] independence=PASS packed-fnv64=0x[0-9a-f]{16}: PASS$' \
            "$output_file" || exit 1
        grep -q '^\[BC6\] mip0/mip1/layer1 signed-mode selection: PASS$' \
            "$output_file" || exit 1
        ;;
